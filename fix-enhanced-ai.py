...
enhanced_ai_code = '''import { EmailContent, ProjectContext, AIChatResponse } from '../../../types/ai.types'
...
<body>
    ...
    try {
      ...
      const enhancedResponse: EnhancedChatResponse = {
        ...
        metadata: {
          ...
          enhancedFeatures: ['smart-intent-detection', 'contextual-suggestions'],
          processingTime: 0,
  ...
  private createFallbackResponse(
    ...
    return {
      response: `Entendi sua mensagem. Por favor, tente reformular ou seja mais específico sobre o que deseja modificar no email.`,
      ...
      suggestions: [
        ...
        'Descreva exatamente o que quer alterar',
  ...
  async analyzePrompt(prompt: string, projectContext?: ProjectContext, userHistory?: UserHistory): Promise<PromptAnalysis> {
    ...
    const confidence = this.calculateConfidence(prompt, intentions)
  ...
  private async callAI(messages: any[], modelOverride?: string): Promise<any> {
    ...
    for (const model of modelsToTry.slice(0, 2)) { // Tentar apenas 2 modelos
      try {
        const controller = new AbortController()
...
Responda APENAS com o JSON válido, sem explicações.`
  ...
  private buildSmartChatPrompt(projectContext: ProjectContext, analysis: PromptAnalysis): string {
    return `Você é um assistente especializado em email marketing. Responda de forma direta e útil sobre modificações em emails.

CONTEXTO:
- Projeto: ${projectContext.projectName}
- Tipo: ${projectContext.type}
- Tom: ${projectContext.tone}

Seja direto, prático e focado em ajudar o usuário a melhorar seu email.`
  }
  ...
  private extractContentRequirements(prompt: string): any {
    const promptLower = prompt.toLowerCase()
  ...
  private calculateConfidence(prompt: string, intentions: any[]): number {
    let confidence = 0.3

    if (prompt.length > 20) confidence += 0.2
    if (intentions.length > 0) confidence += 0.3
    if (prompt.includes('email')) confidence += 0.2
  ...
  private generateSmartSuggestions(message: string, analysis: PromptAnalysis): string[] {
    ...
    suggestions.push('Tente usar palavras como "mude", "altere" ou "troque"')
    suggestions.push('Especifique cores, textos ou elementos que quer alterar')
...
# Aplicar o código
print("Aplicando correções ao Enhanced AI Service...")

# Criar o arquivo
with open('src/services/ai/enhanced/EnhancedAIService.ts', 'w', encoding='utf-8') as f:
    f.write(enhanced_ai_code)

print("✅ Enhanced AI Service corrigido!")
